...
export function handleInput(input: Input): Input {
  // 在此實現函式
  if (typeof input === 'string') return `Input is a string: ${input}`
  if (typeof input === 'number') return `Input is a number: ${input}`;
  return
}
